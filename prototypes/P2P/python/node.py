...
class Node:
	...
	def __init__(self, node, tracker):
		...
		self.sock.bind((node.ipaddr, node.port))
		self.sock.settimeout(1)
		self.tracker_addr = (tracker.ipaddr, tracker.port)
		self.port = node.port
		self.working = True
...
if __name__ == '__main__':
	...
	tracker = TrackerInfo('..\\constants.json')
	node = LocalNodeInfo('..\\constants.json')
	n1 = Node(copy.deepcopy(node), copy.deepcopy(tracker))
	for i in range(node.idletime): sleep(node.idletime)
